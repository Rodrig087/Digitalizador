...
#Desde el archivo configuracion.csv que debe estar presente en el mismo directorio del ejecutable.
        ...
        estaciones_=[]
        with open("configuracion.csv",newline='') as f:
...
def graficar_trama_mseed(nombre_mseed, canal):
    # Lee el archivo mseed y obtiene un objeto Stream
    st = read(nombre_mseed)
    
    # Obtiene la traza del canal especificado
    traza = st[canal - 1]
    
    # Obtiene los valores de tiempo y amplitud de la traza
    tiempo = traza.times()
    amplitud = traza.data
    
    # Crea una figura y un subplot
    fig, ax = plt.subplots(figsize=(8, 6))
    
    # Grafica la traza del canal en el subplot
    ax.plot(tiempo, amplitud)
    ax.set_ylabel(f'Canal {canal}')
...
archivo_binario="/home/rsa/resultados/registro-continuo/230714000000.dat"
parametros=parametros_()
nombre=parametros[1]
...
graficar_trama_mseed(n_mseed,canal)
